...
SmartBot.prototype.onMessageHandler = function (m) {
    ...
    if (m.channel[0] === 'D') {
        switch (m.text) {
            case '-uptime':
                ...
                break;
            case '-status':
                text = 'My Current Configuration\n';
                text += '\nUptime: ' + moment.duration(_.now() - this.startTime).humanize();
                text += '\nLoaded Plugins: ';
                _.each(_this.responder.plugins, function (p) {
                    text += '`' + p.name + '`  :  ';
                });

                text += '\nSlack Token: ' + this.opts.token;
                this._send(m.channel, text);
                done = true;
                break;
        }
    }
    ...
    if (m.user !== this.id) {
        ...
        _this.users.getUser(m.user, function(err, result){
           message.userFirstName = result.profile.first_name;
        });
