...
SmartBot.prototype.onMessageHandler = function (msg) {
    ...
    if (msg.user !== this.id) {
        ...
        message.on('send', function (response) {
            if (response.attachments || response.as_user === false) {
                _this.postMessage(response.channel, response.text, response);
            } else {
                _this.send(response.channel, response.text);
            }
        });
...
SmartBot.prototype.statusMessage = function () {
    ...
    var attachment = {};
    message.attachments = [];
    message.text = 'My current status...';
